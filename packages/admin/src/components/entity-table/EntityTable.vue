...
const { data, isFetching, isError } = useQuery(
  ...
  async () => {
    ...
    if (searchTermDebounced.value !== "")
      params.name = `fts.${encodeURIComponent(searchTermDebounced.value)}:*`;
    const { data, headers } = await postgrestClient.get(
    ...
    if (totalPages.value === 0)
      totalPages.value = Number(headers["content-range"].split("/")[1]);
    return data;
  }
);
...
<template>
  ...
  <div class="px-4 py-4 relative w-full">
    ...
    <ElTable
      ...
      :data="isFetching && !data ? defaultData : data"
      border
      class="relative"
      style="width: 100%"
    >
